...
export const StartGame: React.FC = () => {
  ...
  function switchColor(index: number) {
    setSettingsGame({
      ...settingsGame,
      color: SETTING_GAME_SELECT.color[index].color,
    })
  }
  ...
  function switchTime(index: number) {
    setSettingsGame({
      ...settingsGame,
      time: SETTING_GAME_SELECT.timePlay[index].time,
    })
    if (index > 0) {
  ...
  function switchTimeMove(index: number) {
    setSettingsGame({
      ...settingsGame,
      addTimeMove: SETTING_GAME_SELECT.timePlayMove[index].time,
    })
  }
  ...
  return (
    <PageWrapper layout="alternative" showNav={true} lightColor={true}>
      <WrapperBgColor title="Играть в шахматы с другом на одном компьютере">
        ...
        <Switch
          isWidthAuto={true}
          data={SETTING_GAME_SELECT.timePlay}
          fnAfterClick={switchTime}
        />
        ...
        {isSettingTime && (
          <>
            ...
            <Switch
              isWidthAuto={true}
              data={SETTING_GAME_SELECT.timePlayMove}
              fnAfterClick={switchTimeMove}
            />
          </>
        )}
